  ...
  <v-col xl="2" lg="3" md="3">
    <v-card class="pa-2 c-border-a">
      <v-list-item>
        <v-list-item-content>
          ...
          <v-list-item-title class="headline text-center">
            <span :class="determineColor(highTemp)" v-if="data.maxTemperature.values[0]">
              {{ highTemp }}°
            </span>
            |
            <span :class="determineColor(lowTemp)" v-if="data.minTemperature.values[0]">
              {{ lowTemp }}°
            </span>
          </v-list-item-title>
        </v-list-item-content>
      ...
      <v-list-item class="text-center">
        <v-col cols="12">
          <v-icon color="grey lighten-1" class="wi weather-icon mt-4" size="6vw">
      ...
      <v-list-item class="pa-1 text-center">
        <v-col cols="4">
          <v-icon color="green" size="3vw" class="mr-1 lighten-1">wi-raindrop</v-icon>
          <br />
          {{ calcRainTotal(data.quantitativePrecipitation.values) }}
        </v-col>
        <v-col cols="4">
          <v-icon color="blue" size="3vw" class="mr-1 lighten-1">wi-snowflake-cold</v-icon>
          <br />
          {{ calcSnowTotal(data.snowfallAmount.values) }}
        </v-col>
        <v-col cols="4">
          <v-icon color="white" size="3vw" class="mr-1 grey--text lighten-1">wi-strong-wind</v-icon>
          <br />
  ...
  methods: {
    ...
    determineWeatherIcon(val) {
      ...
      if (val.quantitativePrecipitation.values.length > 0) {
        ...
        precipTotal = (precipTotal / val.quantitativePrecipitation.values.length) * 0.3937
