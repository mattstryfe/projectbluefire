<template>
  <v-container fluid>
    ...
    <!-- Stats AREA -->
    <v-row class="justify-space-around px-2">
      <v-sheet
      ...
      >
        <!-- boost icon -->
        <v-icon size="25" left :color="determineIconColor(key)" class="ma-2">
          {{ determineIcon(key) }}
        </v-icon>
        ...
        <!-- Stat Sheet -->
        <v-sheet v-for="(val, stat) in cat" :key="stat" class="">
          <v-row no-gutters>
            <!-- stat name -->
            ...
            <!-- line spacer -->
            <div
              class="my-auto mx-1 grey darken-3 spacer"
              style="height: 2px"
            />
            ...
            <!-- stat value -->
            <v-col cols="auto">
              <span class="ml-1" :class="determineStatColor(stat, cat)">
...
export default {
  ...
  methods: {
    ...
    updateBoostFilter(stat) {
      ...
      if (ind !== -1) this.boostFilters = boostFilters.filter((b) => b !== stat)
    ...
    determineStatColor(stat, cat) {
      ...
      if (statAndVal.length === 0) return

      if (
        parseInt(cat[stat]) >
        parseInt(cat[stat]) + parseInt(statAndVal[0].adjustments[stat])
      )
        return 'red--text'
