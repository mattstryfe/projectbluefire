<template>
  <v-row no-gutters>
    ...
    <!-- Tabs -->
    <v-sheet class="pa-0 pr-0 col-sm-12 col-md-4 order-sm-1 order-md-2">
      <!--      <MobileMapListToggle v-show="isMobile"/>-->

      <v-tabs v-model="drawer_tab" background-color="primary" dark>
        <v-tab v-for="(tab, ind) in drawerTabs" :key="tab.name">
          <v-badge
          ...
          >
            <v-icon :color="drawer_tab === ind ? 'success' : 'grey darken-2'">
              {{ tab.icon }}
            </v-icon>
          </v-badge>
        </v-tab>
      ...
      <v-tabs-items v-model="drawer_tab">
        <v-tab-item v-for="tab in drawerTabs" :key="tab.name">
          <v-sheet class="overflow-x-hidden px-2" style="height: calc(100vh - 84px)">
...
export default {
  ...
  methods: {
    countResultsIn(tab) {
      if (tab === 'results') return this.$store.state.appointments.length
      if (tab === 'claimed') return this.$store.state.claimedAppointments.length
    },
    isVisible(tab) {
      if (tab === 'form') return false
      if (tab === 'claimed' && this.$store.state.claimedAppointments.length === 0) return false
