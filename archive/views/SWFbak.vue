<template>
  <v-container fluid>
    ...
    <v-row>
      <v-btn color="secondary" small @click="resolveLocation()">Get Weather</v-btn>

      <v-btn class="ml-3" color="secondary" small @click="loadTestData()">load test data</v-btn>
    </v-row>
    <v-row>
      {{ finalWeatherData }}
    </v-row>
...
export default {
  ...
  methods: {
    ...
    resolveLocation() {
      ...
      weatherGovAPI.get(`/points/${locDetails.geo.lat},${locDetails.geo.lng}`).then((res) => {
        weatherGovAPI.get(res.data.properties.forecastGridData).then((res) => {
